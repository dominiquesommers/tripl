...
export class EditableBadge {
  ...
  // NEW: strips the pill background/border/height for contexts (like the
  // cost popup) that want the same input/stepper behavior without the chrome
  ...
  width = computed(() => {
    ...
    return Math.max(totalWidth, 1) + 0.2;
  });

  // Outputs
  save = output<number | null>();           // For the blur/save callback
